...
<Info>
  ¿Certificas e-CF desde la interfaz de Alante (sin API)? Usa la [guía de facturación electrónica](/introduccion/facturacion-electronica). Esta página es para **integradores** que envían comprobantes desde otro sistema.
</Info>
...
| 422 | VALIDATION_ERROR | Datos inválidos | Revisar campos requeridos y formatos (`numero` si `environment` es `testecf`). |
| 422 | NO_BATCH_AVAILABLE | Sin lotes fiscales activos | Configurar lotes E31 (u otro tipo) en Alante — **no aplica** al modo `testecf` + `numero`. |
| 422 | NO_SEQUENCES_AVAILABLE | Sin secuencias en el lote | Solicitar nuevo lote o ampliar secuencias — **no aplica** al modo `testecf` + `numero`. |
...
| 500 | PDF_GENERATION_ERROR / QR_GENERATION_ERROR / XML_NOT_FOUND | Error al generar recurso | Comprobar que el comprobante esté aceptado y que exista XML firmado (flujo estándar). |
