# API de Facturación Electrónica Externa
...
```
https://tu-dominio.com/api/external/fiscal
```

Para desarrollo local:
```
http://localhost:8000/api/external/fiscal
```
...
| `provincia` | string | Provincia del emisor |
...
| `unidad_medida` | string | Sí | Unidad de medida (ej: `UND`, `UN`, `KG`, `MTS`) |
...
```bash
curl -X POST https://tu-dominio.com/api/external/fiscal/invoices \
...
```bash
curl -X GET https://tu-dominio.com/api/external/fiscal/invoices/29d2c2fa-8bf1-4005-88d8-cdbc63e853eb \
  -H "X-API-Key: alante_xxxxxxxxxxxxx"
```
...
```bash
curl -X GET https://tu-dominio.com/api/external/fiscal/invoices/29d2c2fa-8bf1-4005-88d8-cdbc63e853eb/pdf \
  -H "X-API-Key: alante_xxxxxxxxxxxxx" \
  -o factura.pdf
```
