...
- Podrás crear reportes de productos próximos a vencer
...
### 3. Salida de Productos (FIFO Automático)

Cuando vendes productos que requieren lotes, el sistema **automáticamente** aplica el método FIFO (First In, First Out). Esto significa que se descuentan primero los productos del lote más próximo a vencer, sin que tengas que hacer nada.
...
**Ejemplo de funcionamiento FIFO:**
...
El sistema puede alertarte sobre productos que están próximos a vencer (configurable, típicamente 30, 60 o 90 días). Esto te permite tomar acciones preventivas como ofertas especiales o rotación de inventario.
...
- **Historial por lote**: Ver todos los movimientos de un lote específico
- **Reportes detallados**: Analizar rotación, vencimientos y rendimiento por lote

## Reportes Disponibles

El sistema ofrece varios reportes relacionados con lotes:

1. **Reporte de Productos Próximos a Vencer**
   - Lista productos con lotes que vencen en un período determinado
   - Filtros por días de anticipación, almacén y producto
   - Útil para planificar rotación de inventario

2. **Reporte de Productos Vencidos**
   - Lista productos con lotes que ya han vencido
   - Muestra cantidad vencida por producto
   - Ayuda a identificar productos que deben retirarse

3. **Kardex por Lote**
   - Historial completo de movimientos de un lote específico
   - Muestra entradas, salidas y ajustes
   - Trazabilidad completa del lote

4. **Rotación por Lote**
   - Análisis de qué lotes se están vendiendo primero
   - Identifica lotes estancados que requieren atención
   - Útil para optimizar la gestión de inventario
...
1. **Automatización**: El FIFO se aplica automáticamente, sin intervención manual
...
5. **Control**: Alertas y reportes que te ayudan a gestionar vencimientos eficientemente
...
- **Seguridad**: El sistema usa identificadores únicos (UUID) para proteger la información
...
- Asegurar rotación adecuada (FIFO automático)
